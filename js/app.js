...
let generatorBtnElem = document.querySelector('button')
let resultElem = document.querySelector('#result')

generatorBtnElem.addEventListener('click' , function(event){
  event.preventDefault()
  let amountInputValue = amountInputElem.value 
  let randomTextIndex = Math.floor(Math.random()*dummyText.length)
  if (amountInputValue < 0 || amountInputValue > 9 || amountInputValue === '' || isNaN(amountInputValue)) {
    resultElem.innerHTML=dummyText[randomTextIndex]
  } else{
    let slicedArray = dummyText.slice(0,amountInputValue)
    let finalTextsArry =slicedArray.map(function(item){
      return '<P>'+item+'</p>'
    }).join('')
    resultElem.innerHTML = finalTextsArry
  }
})